...
class MatchesWholeCycleUnitTesting {
   ...
   /* @ExperimentalCoroutinesApi
    @Test
    fun test_generate_invoice_pdf_success() = testCoroutineRule.runBlockingTest {
        val invoiceUrl = InvoiceUrl("")
        val apiResult = ApiResponse(data = invoiceUrl, message = "", total = 0, success = true)

        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.generateInvoicePdf(
        ...
        viewModel.generateInvoicePdf(5)

        Mockito.verify(loadingObserver).onChanged(true)
        Mockito.verify(invoiceUrlObserver).onChanged(apiResult.data.invoiceLink)
        Mockito.verify(loadingObserver).onChanged(false)
        Mockito.verifyNoMoreInteractions(invoiceUrlObserver, loadingObserver)
    }

    @ExperimentalCoroutinesApi
    @Test
    fun test_generate_invoice_pdf_Failure() = testCoroutineRule.runBlockingTest {
        val exception = RuntimeException("Failed")
        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.generateInvoicePdf(
        ...
        Mockito.verify(loadingObserver).onChanged(true)
        Mockito.verify(errorObserver).onChanged(Mockito.any(java.lang.RuntimeException::class.java))
        Mockito.verify(loadingObserver).onChanged(false)
        Mockito.verifyNoMoreInteractions(loadingObserver, errorObserver)
    }

    @ExperimentalCoroutinesApi
    @Test
    fun test_cancel_shipment_success() = testCoroutineRule.runBlockingTest {
        val apiResult = ApiResponse(data = Any(), message = "", total = 0, success = true)

        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.cancelShipment(
                ArgumentMatchers.anyInt(),
                ArgumentMatchers.anyString()
            )
        )
        ...
        viewModel.cancelShipment(5)

        Mockito.verify(loadingObserver).onChanged(true)
        Mockito.verify(cancelShipmentSuccessObserver).onChanged(true)
        Mockito.verify(loadingObserver).onChanged(false)
        Mockito.verifyNoMoreInteractions(invoiceUrlObserver, loadingObserver)
    }
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun test_cancel_shipment_Failure() = testCoroutineRule.runBlockingTest {
        val exception = RuntimeException("Failed")
        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.cancelShipment(
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun test_validate_shipment_cancellation_success() = testCoroutineRule.runBlockingTest {
        val result = ValidationResponse(1,"")

        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.validateShipmentCancellation(
                ArgumentMatchers.anyInt(),
                ArgumentMatchers.anyString()
            )
        )
        ...
        viewModel.validateShipmentCancellation(5)

        Mockito.verify(loadingObserver).onChanged(true)
        Mockito.verify(validateShipmentCancellationObserver).onChanged(result)
        Mockito.verify(loadingObserver).onChanged(false)
        Mockito.verifyNoMoreInteractions(invoiceUrlObserver, loadingObserver)
    }
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun test_validate_shipment_cancellation_Failure() = testCoroutineRule.runBlockingTest {
        val exception = RuntimeException("Failed")
        Mockito.`when`(localDataSource.getUserType()).thenReturn("retailer")
        Mockito.`when`(
            orderHistoryRemoteDataSource.validateShipmentCancellation(
